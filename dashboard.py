...
def get_ai_analysis(current_price, max_pain, gex_df, dte, series_name, vol_call_df=None, vol_put_df=None):
    ...
    prompt = f"""บทบาทและตรรกะหลัก (Role & Core Logic):
        ...
        if not df_call.empty and not df_put.empty:
            ...
            with col2:
                ...
                if not df_vol_call.empty and not df_vol_put.empty:
                    ...
                    if current_price > 0:
                        fig_vol.add_vline(x=current_price, line_dash="dash", line_color="white")
            ...
            with col3:
                ...
                if current_price > 0:
                    fig_pain.add_vline(x=current_price, line_dash="dot", line_color="white")
            ...
            if eod_times:
                ...
                if eod_raw.get("points"):
                    ...
                    vol_df = eod_df[ (eod_df["DataType"] == "Volatility") | (eod_df["SeriesName"].str.contains("Vol", case=False)) | (eod_df["SeriesName"].str.contains("Settle", case=False)) ]
                    if not vol_df.empty:
                        ...
                        if current_price > 0:
                            fig_vol.add_vline(x=current_price, line_dash="dot", line_color="white", annotation_text="Spot")
            ...
            if st.button("✨ กดเพื่อให้ AI วิเคราะห์แผนการเทรด"):
                with st.spinner(f"🧠 AI ({GROQ_MODEL}) กำลังวิเคราะห์... (Powered by Groq)"):
                    ai_insight = get_ai_analysis(
                        current_price, max_pain_strike, gex_df, dte, selected_series, df_vol_call, df_vol_put
                    )
